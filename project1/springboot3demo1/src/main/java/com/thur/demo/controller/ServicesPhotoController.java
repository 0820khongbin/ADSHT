...
import com.thur.demo.service.ServicesPhotoService;


@RequestMapping("/ajax/pages/ServicesPhoto")
@RestController //REST controller是透過REST，以JSON格式與前端溝通。
@CrossOrigin
public class ServicesPhotoController {

	@Autowired
	private ServicesPhotoService servPhotoServ;

	//新增
	@PostMapping("/add")
	public String create(@RequestBody String infoBody) {
		JSONObject responseJson = new JSONObject();
		JSONObject obj = new JSONObject(infoBody);
		String photo = obj.isNull("photo") ? null : obj.getString("photo");
        if(photo == null) {
        	responseJson.put("success", false);
        	responseJson.put("message", "photo是必要欄位");
        } else {
                ServicesPhoto servPhoto = servPhotoServ.insert(infoBody);
                if(servPhoto==null) {
                	responseJson.put("success", false);
