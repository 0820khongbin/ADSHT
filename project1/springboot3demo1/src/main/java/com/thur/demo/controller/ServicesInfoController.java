...
public class ServicesInfoController {
        ...
        if(id==null) {
            ...
            if(servInfoServ.findById(id) == null) {
                ...
                if(!servInfoServ.remove(id)) {
                	responseJson.put("success", false);
                	responseJson.put("message", "尚有會員未退訂");
                } else {
                	responseJson.put("success", true);
        ...
        if(servInfos!=null && !servInfos.isEmpty()) {
            for(ServicesInfo servInfo : servInfos) {
                JSONObject item = new JSONObject()
                        ...
                        .put("ServiceLevel", servInfo.getLevel())	
  ...
  //查詢我的訂閱
    ...
    public String findMySubsById( @RequestHeader("session-id") UUID loginUserId) {
        ...
        if(servInfos!=null && !servInfos.isEmpty()) {
            for(ServicesInfo servInfo : servInfos) {
            	List<Subscriptions> subscriptions = servInfo.getSubscriptions();
            	 for(Subscriptions subscription : subscriptions) {
                     JSONObject item = new JSONObject()
                             .put("ServiceId", servInfo.getServiceId())
                             .put("ServiceName", servInfo.getServiceName())
                             .put("ServiceCreater", subscription.getUser().getUserinfo().getUserLastName())
                             .put("ServiceCreaterId", subscription.getUser().getUserinfo().getId())
                             .put("ServiceCreaterImg", subscription.getUser().getUserinfo().getPhoto())
                             .put("Price", servInfo.getPrice())
                             .put("Description", servInfo.getDescription())
                             .put("ServiceLevel", servInfo.getLevel())
                             .put("Image", servInfo.getImage())
    ...
    //查詢我的訂閱專屬圖片
    @PostMapping("/findMySubsPhotoById")
    public String findMySubsPhotoById( @RequestHeader("session-id") UUID loginUserId) {
        JSONObject responseJson = new JSONObject();
        JSONArray array = new JSONArray();
        List<ServicesInfo> servInfos = servInfoServ.findMySubsById(loginUserId);
        if(servInfos!=null && !servInfos.isEmpty()) {
            for(ServicesInfo servInfo : servInfos) {
            	List<Subscriptions> subscriptions = servInfo.getSubscriptions();
            	 ...
            	 for(Subscriptions subscription : subscriptions) {
                     ...
                     List<ServicesPhoto> photos = servInfo.getServicesPhoto();
                     if(photos != null && !photos.isEmpty()) {
                    	 for( ServicesPhoto photo : photos ) {
                           	 JSONObject itemPhoto = new JSONObject()
                         			 	.put("ServicesPhoto", photo.getServPhoto());
                         	 array2 = array2.put(itemPhoto);
                         	 }
                    	 }else {
                    		 JSONObject itemPhoto = new JSONObject();
                    		 array2 = array2.put(itemPhoto);
