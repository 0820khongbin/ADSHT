    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h1 class="modal-title fs-5" id="staticBackdropLabel">修改你提供的會員項目</h1>
        ...
        <div class="modal-body">
          <div class="container group">
            <!-- <div class="grid-1-5"> -->
            <div>
              <img v-if="modelValue.Image" :src="modelValue.Image" alt="" width="80px" height="80px"
                @dblclick="triggerFileInput" style="cursor: pointer">
              <img v-else src="/public/favicon.ico" alt="" width="80px" height="80px" @dblclick="triggerFileInput">
              <span v-if="serviceImageTemp">
                <font-awesome-icon :icon="['fas', 'arrow-rotate-left']" class="icon-large" v-if="serviceImageTemp"
                  @click="clearImage" /></span>
              <br><br><br>
              <table>
                <tr>
                  <td>訂閱名稱 : </td>
                ...
                <tr>
                  <td>價格 : </td>
                  <td><input type="number" :value="modelValue.Price" @input="doinput('Price', $event)"></td>
                </tr>
              </table>
              <br>
              <div class="form-group">
                <label for="serviceLevel">會員等級:&nbsp;&nbsp;&nbsp;&nbsp;</label>
                <span>LV {{ modelValue.ServiceLevel }}</span>
                <br>
                <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
                  <label v-for="level in levels" class="btn btn-outline-primary" :for="'btnradio' + level">等級 {{ level
                    }}</label>
                  <input v-for="level in levels" type="radio" class="btn-check" :id="'btnradio' + level" :value="level"
                    name="serviceLevel" @input="doinput('ServiceLevel', $event)" autocomplete=" off">
                </div>
              </div>
            </div>
            <!-- </div> -->
...
const serviceImageTempFlag = ref("");
...
const onFileChange = (e) => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      ...
      if (serviceImageTempFlag.value != 1) {
        serviceImageTemp.value = props.modelValue.Image;
        serviceImageTempFlag.value = 1;
      }
      emits('update:modelValue', {
        ...props.modelValue,
...
const clearImage = () => {
  emits('update:modelValue', {
    ...props.modelValue,
    Image: serviceImageTemp.value
  });
  serviceImageTemp.value = "";
  serviceImageTempFlag.value = 0;
};
