  ...
  <div class="nft">
    <div class='main'>
      <RouterLink :to="`shop?id=${shop.shopId}`">
        ...
        <h2 style="text-align: center;">{{ shop.shopName }}</h2>
        <hr>
        <p class='description'>商店簡介: {{ shop.description }}</p>
        <div class='tokenInfo'>
          <div class="price">
          </div>
          <div class="duration">
        ...
        <div class='creator'>
          <div class='wrapper'>
            <img :src="userInfo.photo || defaultAvatar" alt="Creator" loading="lazy" style="width: 30px;height: 30px;"/>
          </div>
          <p style="display: inline;"><ins>Creation of</ins> {{ userInfo.userFirstName }} {{ userInfo.userLastName }}</p>
        </div>
