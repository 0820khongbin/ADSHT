<template>
  <div class="body">
    <div class="profile">
      <div class="forColor">
        <div class="photo"><img :src="post.posterPhoto" /></div>
        <div class="content">
          <div class="text">
            <RouterLink :to="`/views/post?id= + ${post.postId}`">
              <h3>{{ post.posterName }}</h3>
              <h6>{{ post.content }}</h6>
            </RouterLink>
          </div>
          <button class="btn" :id="post.posterId" @click="showTog"><span></span></button>
        </div>
        <div class="box"><img v-if="post.image" :src="post.image">
          <i class="heart outline like icon"></i>
          <i><font-awesome-icon :icon="['fab', 'rocketchat']" /></i>
          <i><font-awesome-icon :icon="['fas', 'user']" /></i>
          <i><font-awesome-icon :icon="['fas', 'bomb']" /></i>
          <i><font-awesome-icon :icon="['fas', 'cart-shopping']" /></i>
        </div>
      </div>
    </div>
  </div>
...
function showTog(event) {
  const btn = $(event.target).closest('.btn');
  const box = btn.closest('.content').siblings('.box');
